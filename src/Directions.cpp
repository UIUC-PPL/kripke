...
void InitDirections(Grid_Data *grid_data, Input_Variables *input_vars)
{
  ...
  if(num_polar > 0){
    ...
    if(num_polar % 4 != 0){
      CkAbort("Must have number of polar angles be a multiple of 4\n");
    }
    if(num_azimuth % 2 != 0){
      CkAbort("Must have number of azimuthal angles be a multiple of 2\n");
    }
    if(num_polar*num_azimuth != num_directions){
      CkPrintf("You need to specify %d total directions, not %d\n",
          num_polar*num_azimuth, num_directions);
      CkAbort("Number of directions is invalid\n");
    }
