...
*/
void Subdomain::setup(int zsIdx[3], int sdom_id, Input_Variables *input_vars, 
        int gs, int ds, int zs, std::vector<Directions> &direction_list, Kernel *kernel)
{
  ...
  for(int dim = 0;dim < 3;++ dim){
    ...
    nzones[dim] = getNumZones(dim, zsIdx[dim], input_vars);
    num_zones *= nzones[dim];

    // Compute grid deltas in this dimension (including ghost zone deltas)
    std::pair<double, double> dim_extent = getSpatialExtents(dim, zsIdx, input_vars->num_zonesets_dim);
  ...
  for(int dim = 0;dim < 3;++ dim){
    downwind[dim] = getNeighbor(false, zsIdx, input_vars->num_zonesets_dim, sdom_id, dim, dirs[dim]);
    upwind[dim]   = getNeighbor(true, zsIdx, input_vars->num_zonesets_dim, sdom_id, dim, -1 * dirs[dim]);
  }
...
/*
 * Return a neighbor for communication, upwind or downwind.
 */
Neighbor Subdomain::getNeighbor(bool upwind, int zsIdx[3], int zsDims[3], 
        int sdom_id, int dim, int dir) {
  
  Neighbor n;
  int nborIdx[3] = {zsIdx[0], zsIdx[1], zsIdx[2]};
  nborIdx[dim] += dir;

  if (nborIdx[dim] < 0 || nborIdx[dim] >= zsDims[dim]) {
    n.x = -1; 
    n.y = -1; 
    n.z = -1;
  }
 ...
 */
std::pair<double, double> Subdomain::getSpatialExtents(int dim, int zsIdx[3], int zsDims[3]) const{
  ...
  // Subdivide by number of zone sets (chares) in specified dimension
  double dx = (ext_global.second - ext_global.first) / (double)zsDims[dim];
  std::pair<double, double> ext_proc(
    ext_global.first + dx*(double)zsIdx[dim],
    ext_global.first + dx*(double)(zsIdx[dim] + 1)
  );
...
/*
 * Return the number of zones in the given dimension.
 */
int Subdomain::getNumZones(int dim, int zsIdx, Input_Variables *input_vars) {
 
  int global_zones, global_zonesets = input_vars->num_zonesets_dim[dim];

  if (dim == 0)
    global_zones = input_vars->nx;
  else if (dim == 1)
    global_zones = input_vars->ny;
  else
    global_zones = input_vars->nz;
  
  int num_zones = global_zones / global_zonesets;
  int rem = global_zones % global_zonesets;
  if (rem != 0 && zsIdx < rem) {
    num_zones ++;
  }
...
namespace {
  ...
  inline double PnmFcn(int n, int m, double x)
  {
    ...
    if(std::abs(x) > 1.0){
      CkPrintf("Bad input to ardra_PnmFcn: abs(x) > 1.0, x = %e\n", x);
      CkAbort("Bad input to ardra_PnmFcn\n");
    }
    else if((x > 1.0) && (x <= 1.0)){
      x = 1.0;
    }
