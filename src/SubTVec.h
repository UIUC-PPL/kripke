...
public:
  ...
  inline void pup(PUP::er &p) {
    PUParray(p, ext_to_int, 3);
    PUParray(p, size_int, 3);
    p|groups;
    p|directions;
    p|zones;
    p|elements;
    ...
    p|data_linear;
    if (aliasing) {
      if (p.isUnpacking()) data_pointer = new double[elements];
      PUParray(p, data_pointer, elements);
    }
    else {
      if (p.isUnpacking()) data_pointer = &data_linear[0];
    }
  }
};
